...
def compute_tre(vector,target):
    ...
    while norm(unit(vector[j]-centroid))==0:
        j=j+1
        
    
    i = j+1
    while norm(np.cross(unit(vector[j]-centroid),unit(vector[i]-centroid)))==0:
        i = i+1 
    temp_vec =  unit(vector[j] -  centroid)
    temp_vec2 = np.cross(temp_vec,unit(vector[i]-centroid))
...
def distanceFromLine(p1,p2,p3):
    ...
    if norm(p2-p1) == 0:
        ...
        d = norm(np.cross(p2-p1, p3-p1))/norm(p2-p1)
        return d
...
def compute_intermarker(f):
    fiducial_count = len(f)
